...
void OdometrySpoofNode::timerCallback() {
  ...
  // Fill header
  odom_msg.header.stamp = this->get_clock()->now();//transform_stamped.header.stamp;
  ...
  // RCLCPP_INFO(this->get_logger(), "Odom published: %d s, %d nanos", odom_msg.header.stamp.sec, odom_msg.header.stamp.nanosec);
  // RCLCPP_INFO(this->get_logger(), "Transform Position - X: %.3f, Y: %.3f",
